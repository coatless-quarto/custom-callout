...
- Fixed FontAwesome icons rendering as placeholder characters by bundling FA 7 Free Solid font (woff2) and registering it as an HTML dependency.
...
- Specify the custom callout by using a unique key under the `custom-callout` header and setting the following properties:
  - `icon-symbol`: Set the icon symbol for the callout using a valid string, emoji, or [FontAwesome 7](https://fontawesome.com/search?o=r&m=free) icon.
